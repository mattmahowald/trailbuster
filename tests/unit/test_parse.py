...
from salesforce.parse import (
    ...
    _extract_learning_objectives,
    _extract_lesson_content,
    _extract_lessons_list,
    _extract_links,
    _extract_prerequisites,
    _extract_time_estimate,
    _extract_title,
    parse_lesson,
    parse_module,
)
...
class TestParseModule(unittest.TestCase):
    ...
    def test_parse_lesson_with_mock_data(self):
        ...
        self.assertIn(
            "Understand what the Salesforce Platform is", result.learning_objectives
        )
        self.assertIn(
            "Learn about custom objects and fields", result.learning_objectives
        )
        self.assertIn(
            "Explore the App Exchange marketplace", result.learning_objectives
        )
        ...
        self.assertIn(
            "https://trailhead.salesforce.com/content/learn/modules/platform_dev_basics",
            link_urls,
        )
    ...
    def test_extract_time_estimate_text_pattern(self):
        ...
        self.page.set_content(html_content)
        time_estimate = _extract_time_estimate(self.page)
        # The regex pattern captures just the number and unit, so "25 minutes" becomes "25 min"
        self.assertEqual(time_estimate, "25 min")
    ...
    def test_content_item_dataclass(self):
        ...
        item2 = ContentItem(text="Test heading", element_type="heading", level=2)
        self.assertEqual(item2.level, 2)

        # Test content item with URL
        item3 = ContentItem(
            text="Test link", element_type="link", url="https://example.com"
        )
